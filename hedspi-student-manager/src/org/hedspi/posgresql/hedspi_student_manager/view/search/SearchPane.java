...
public class SearchPane extends JPanel {
	...
	public SearchPane() {
		...
		JLabel lblSearchString = DefaultComponentFactory.getInstance()
		...
		JLabel lblNresultsThreshold = DefaultComponentFactory.getInstance()
				.createLabel("NResults threshold");
		panel.add(lblNresultsThreshold, "2, 4");

		JSpinner spinner = new JSpinner();
		model = new SpinnerNumberModel(new Integer(10), new Integer(0), null,
				new Integer(1));
		...
		JCheckBoxMenuItem chckbxmntmLecturerDegree = new JCheckBoxMenuItem(
				"Degrees");
		chckbxmntmLecturerDegree.setSelected(true);
		popupMenu_4.add(chckbxmntmLecturerDegree);

		JCheckBoxMenuItem chckbxmntmLecturerFaculty = new JCheckBoxMenuItem(
				"Faculty");
		chckbxmntmLecturerFaculty.setSelected(true);
		popupMenu_4.add(chckbxmntmLecturerFaculty);

		resultModel = new DefaultTableModel(new Object[][] {}, new String[] {
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// City
				City.setSearchStringArg(chckbxmntmCityName.isSelected(),
						chckbxmntmCityDistricts.isSelected());
				...
				// result
				HedspiObject[] rets = searchContainer.search(
						textField.getText(), model.getNumber().intValue(),
						chckbxCaseSensitive.isSelected());
				for (int i = resultModel.getRowCount() - 1; i >= 0; i--)
					resultModel.removeRow(i);
				int cnt = 1;
				for (HedspiObject it : rets) {
					resultModel.addRow(new Object[] { cnt++, it.getClassName(),
							it, it.getSearchString() });
				}
			}
		});
