...
public class AddressService {
	public static Pair<HedspiObjects<City>, HedspiObjects<District>> getAddresses() {
		Control.getInstance().getLogger()
				.log(Level.INFO, "Fetch list of cities from database");

		// list of cities
		Control.getInstance().getLogger()
				.log(Level.INFO, "Fetch list of cities");
		...
		Control.getInstance().getLogger()
				.log(Level.INFO, "Fetch list of districts");
		...
		for (HashMap<String, Object> it : rs) {
			...
			District district = new District(
					String.valueOf((int) it.get("DT#")), name, city);
			city.getDistricts().put(district);
			districts.put(district);
		}

		return new Pair<HedspiObjects<City>, HedspiObjects<District>>(cities,
				districts);
	}
